...
pub extern "C" fn private_encode_contact_trace(
    ...
    // decryptedは復号化したバイナリ、dictionary_bufferがサーバ側のデータを持っている
...
pub extern "C" fn get_encoded_result(
    ...
    // ここで、response_vecの中身を生成してる、暗号化を楽にするためにバイナリしてる
    result_buffer.build_query_response(&query_buffer, &mut response_vec);

    /* encryption */
    // ここでappに送るレスポンスを暗号化してる
    ...
    for (i, query_rep) in query_buffer.queries.iter().enumerate() {
        ...
        // rsgx_aes_ctr_encryptはもともとあるメソッドで、これの引数はバイナリだから
        let ret = rsgx_aes_ctr_encrypt(
            &shared_key,
            // クエリidを暗号化してない、current_cursorは現在書き込みたいbufferの位置、結果だけ暗号化してる。
